...
int main(int argc, char *argv[])
{
	...
	gtk_window_set_title(GTK_WINDOW(window), "채팅프로그램");
	gtk_container_set_border_width(GTK_CONTAINER(window), 15);
	table =gtk_table_new(8,4,FALSE);
	gtk_table_set_col_spacings(GTK_TABLE(table), 3);

	//내부 
	title = gtk_label_new ("채팅프로그램");
	...
	//파일찾기
	activate = gtk_button_new_with_label("파일찾기");
	...
	close = gtk_button_new_with_label("전송");
	...
	help =gtk_button_new_with_label("보내기");
	...
	//확인
	ok = gtk_button_new_with_label("확인");
